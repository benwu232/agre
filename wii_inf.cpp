...
int button1_flag = 0, button_b_flag = 0, static_gesture_flag = 0;
...
/*
        ...
        QString fn = TimeStr + ".tst";

        QByteArray ba1 = fn.toLatin1();
        const char *cstr1 = ba1.data();

        fp = fopen(cstr1, "w");
        for (k = 0; k < PointNum; k++)
            fprintf(fp, "%10f%10f%10f\n", AccelArray[k].x, AccelArray[k].y, AccelArray[k].z);
        fclose(fp);
        printf("File writing over!\n");
...
*/
        ...
        for (k = 0; k < PointNum; k++)
        {
            AccX[k] = AccelArray[k].x;
            AccY[k] = AccelArray[k].y;
            AccZ[k] = AccelArray[k].z;
        }
        //mutex.unlock();
    ...
    {
        wiiuse_set_leds(wm, WIIMOTE_LED_4 | WIIMOTE_LED_1);
    ...
    */
    ...
    if (WIIUSE_USING_ACC(wm)) {
        ...
        */
        //printf("wiimote gforce = %10f%10f%10f\n", wm->gforce.x, wm->gforce.y, wm->gforce.z);
        if (button1_flag == 1 || button_b_flag == 1)
        {
            AccelArray[PointNum].x = wm->gforce.x;
            AccelArray[PointNum].y = wm->gforce.y;
            AccelArray[PointNum].z = wm->gforce.z;
            if (PointNum < MAX_AA)
                PointNum++;
        }
        if ( button_b_flag == 1)
        {
            w->StaticGesturePro(AccelArray);

            if (wm->gforce.x > SLOPE)
                wiiuse_set_leds(wm, WIIMOTE_LED_4);
            else
                wiiuse_set_leds(wm, WIIMOTE_LED_NONE);

            if (wm->gforce.x < 0 - SLOPE)
                wiiuse_set_leds(wm, WIIMOTE_LED_1);
            else
                wiiuse_set_leds(wm, WIIMOTE_LED_NONE);

            if (wm->gforce.y > SLOPE)
                wiiuse_set_leds(wm, WIIMOTE_LED_2);
            else
                wiiuse_set_leds(wm, WIIMOTE_LED_NONE);

            if (wm->gforce.y < 0 - SLOPE)
                wiiuse_set_leds(wm, WIIMOTE_LED_3);
            else
                wiiuse_set_leds(wm, WIIMOTE_LED_NONE);
...
int WiimoteInit()
{
    ...
    */
    ...
    // Set wiimote parameters
    wiiuse_set_orient_threshold(wiimotes[0], 30.0f);
